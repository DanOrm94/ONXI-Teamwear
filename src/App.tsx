    ...
    <div className={`product-visual ${product.imageClass}`} aria-label={product.name}>
      {product.category === 'Kits' ? (
        <div className="jersey">
          ...
          <span className="jersey-sleeve left" />
          <span className="jersey-sleeve right" />
          <span className="jersey-mark">ONXI</span>
      ...
      <main id="top">
        <section className="hero">
          ...
          <div className="hero-art" aria-hidden="true">
            ...
            <div className="hero-jersey"><span>ONXI</span><strong>10</strong></div>
            <div className="hero-word">ONXI</div>
          </div>
        </section>
        ...
        <section className="shop-section" id="shop">
          <div className="section-heading">
            <div><p className="eyebrow">02 / THE COLLECTION</p><h2>SHOP <em>ONXI</em></h2></div>
            <div className="filters">{(['All', 'Kits', 'Grip Socks'] as const).map((item) => <button className={category === item ? 'active' : ''} key={item} onClick={() => setCategory(item)}>{item}</button>)}</div>
          </div>
          <div className="product-grid">
            {filtered.map((product) => (
              <article className="product-card" key={product.id}>
                <button className="product-hit" onClick={() => { setSelected(product); setSize(product.sizes[0]) }}>
                  <div className="badge-row">{product.badge && <span className="badge">{product.badge}</span>}<span>{product.category}</span></div>
                  <ProductVisual product={product} />
                  <div className="product-info"><div><h3>{product.name}</h3><p>{product.colour}</p></div><strong>{money(product.price)}</strong></div>
                </button>
                <button className="quick-add" onClick={() => addToCart(product, product.sizes[0])}>Quick add <span>+</span></button>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="feature" id="story">
          <div className="feature-art"><div className="feature-socks"><span /><span /></div><span className="feature-label">GRIP / 01</span></div>
          <div className="feature-copy"><p className="eyebrow">03 / LOCKED IN</p><h2>FEEL THE<br /><em>DIFFERENCE.</em></h2><p>Our grip socks are built around one idea: keep your foot exactly where you put it. Targeted silicone grip zones help reduce movement inside the boot, so you can focus on the next touch.</p><div className="spec-list"><div><strong>01</strong><span>High-friction grip zones</span></div><div><strong>02</strong><span>Breathable performance knit</span></div><div><strong>03</strong><span>Arch support fit</span></div></div><button className="text-button" onClick={() => { setCategory('Grip Socks'); document.getElementById('shop')?.scrollIntoView({ behavior: 'smooth' }) }}>Shop grip socks <span>↗</span></button></div>
        </section>
